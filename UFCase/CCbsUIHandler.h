...
namespace winrt::UFCase {
    ...
    {
        CCbsUIHandler(
            std::function<void(uint32_t)> const &progress,
            std::function<void(hresult)> const &error): OnProgress(progress), OnError(error) { }
        ...
        HRESULT STDMETHODCALLTYPE ResolveSource(PCWSTR, ICbsIdentity*, PCWSTR, PWSTR*, int*) override {
            return S_OK;
        }
