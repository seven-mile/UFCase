...
template <class FuncT>
    requires requires {
        requires std::is_function_v<std::remove_pointer_t<FuncT>>;
    }
[[nodiscard]] static FuncT GetFunction(const std::wstring module,
...
std::wstring FindWcpDllOnline()
{
    ...
    auto pfnSssBindServicingStack =
        GetFunction<PSSS_BIND_SERVICING_STACK_FUNCTION>(
    ...
    SSS_BIND_PARAMETERS param{.cbSize = sizeof(param),
                              .dwFlags =
                                  SSS_BIND_CONDITION_FLAGS_ARCHITECTURE |
    ...
    DWORD dwDisposition;
    THROW_IF_FAILED(pfnSssBindServicingStack(&param, wil::out_param(pCookie),
                                             &dwDisposition));

    std::wstring_view strvLocation{GetViewFromLUC(pCookie->ucLocation)};

    UINT64 lenPath = 0;
    winrt::check_hresult(pfnSssGetServicingStackFilePathLength(
        0, pCookie.get(), WCP_DLL_FILENAME, &lenPath));
    assert(lenPath > 0);
    auto bufPath = std::make_unique<wchar_t[]>(lenPath);
    UINT64 gotLen = 0;
    winrt::check_hresult(
        pfnSssGetServicingStackFilePath(0, pCookie.get(), WCP_DLL_FILENAME,
...
inline void EnsureBackupRestorePrivilege()
{
    ...
    winrt::check_bool(AdjustTokenPrivileges(hToken.get(), FALSE, &tp,
...
WcpGuard::WcpGuard(winrt::com_ptr<IMalloc> pMalloc)
{
    ...
    auto pfnWcpInitialize = GetFunction<PWCP_INITIALIZE_FUNCTION>(
        WCP_DLL_FILENAME, "WcpInitialize");
...
std::pair<winrt::com_ptr<IStore2>, winrt::com_ptr<ICSIStore>> LoadOnlineStores()
{
    ...
    auto pfnGetSystemStore = GetFunction<PGET_SYSTEM_STORE_FUNCTION>(
        WCP_DLL_FILENAME, "GetSystemStore");

    winrt::check_hresult(
        pfnGetSystemStore(0, __uuidof(IStore), (::IUnknown **)pStore.put()));
    winrt::check_hresult(pfnGetSystemStore(0, __uuidof(ICSIStore),
                                           (::IUnknown **)pCSIStore.put()));
    ...
    std::filesystem::path windir)
{
    ...
    auto pfnOpenExistingOfflineStore =
        GetFunction<POPEN_EXISTING_OFFLINE_STORE_FUNCTION>(
            WCP_DLL_FILENAME, "OpenExistingOfflineStore");

    const std::wstring OFFLINE_HIVE_PREFIX =
        L"{bf1a281b-ad7b-4476-ac95-f47682990ce7}"s;
    ...
    const auto reg_hive_names = {
        L"SOFTWARE"s, L"SYSTEM"s,     L"SECURITY"s,
        L"SAM"s,      L"COMPONENTS"s, L"DEFAULT"s,
    };
    ...
    for (auto &hive_name : reg_hive_names)
    {
        ...
        winrt::check_nt(
            RegOpenKey(HKEY_LOCAL_MACHINE, offreg.c_str(), hive.put()));
        ...
        if (hive_name == L"SOFTWARE")
        {
            ...
            winrt::check_nt(RegQueryValueEx(
                hkProfileList.get(), L"ProfilesDirectory", nullptr, &dwType,
                (LPBYTE)szProfileDir, &dwLen));
            ...
            auto ntuser_offreg =
                OFFLINE_HIVE_PREFIX + ntuser_hive_path.wstring();
            fix_offline_hive_key_name(ntuser_offreg);

            wil::unique_hkey ntuser_hive;
            winrt::check_nt(RegOpenKey(
                HKEY_LOCAL_MACHINE, ntuser_offreg.c_str(), ntuser_hive.put()));
    ...
    OFFLINE_STORE_CREATION_PARAMETERS params{
        ...
        .pszHostRegistryMachineSoftwarePath =
            hive_offregs[L"SOFTWARE"s].c_str(),
        .pszHostRegistryMachineSystemPath = hive_offregs[L"SYSTEM"s].c_str(),
        .pszHostRegistryMachineSecurityPath =
            hive_offregs[L"SECURITY"s].c_str(),
        .pszHostRegistryMachineSAMPath = hive_offregs[L"SAM"s].c_str(),
        .pszHostRegistryMachineComponentsPath =
            hive_offregs[L"COMPONENTS"s].c_str(),
        ...
        .pszHostRegistryMachineOfflineSchemaPath =
            hive_offregs[L"SCHEMA"s].c_str(),
    };

    DWORD dwDisp{};
    check_hresult(pfnOpenExistingOfflineStore(
        0, &params, __uuidof(IStore2), (::IUnknown **)pStore.put(), &dwDisp));
...
StoreModel::StoreModel(ImageModel *image, com_ptr<IStore2> sxs_store,
    ...
    : image(*image), sxs_store(sxs_store), csi_store(csi_store),
      wcp_guard(wcp_guard)
{
}
...
StoreModel *StoreModel::Create(ImageModel *image)
{
    ...
    {
        std::tie(pStore, pCSIStore) =
            WcpUtil::LoadOfflineStores(image->WinDir());
    }
