...
ImageModel::ImageModel(std::filesystem::path const &bootdrive)
    : bootdrive(bootdrive)
{
...
{
    return this->bootdrive == GetOnlineBootdrive() ? ImageType::Online
                                                   : ImageType::Offline;
}
...
bool ImageModel::IsWinPE()
{
    if (this->Type() == ImageType::Online)
    {
        ...
        auto lst = ::RegOpenKey(HKEY_LOCAL_MACHINE,
                                L"Microsoft\\Windows NT\\CurrentVersion\\WinPE",
                                wil::out_param(hkey));
        SetLastError(0);
